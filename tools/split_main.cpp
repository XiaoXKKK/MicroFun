...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "-i" && i + 1 < argc)
      ...
      std::cout
          << "Usage: split_tool -i <input_map.png> -o <output_dir> [options]\n";
      std::cout << "Options:\n";
      std::cout << "  --quadtree              Use quad-tree splitting based on "
                   "color uniformity\n";
      std::cout
          << "  --max-depth <depth>     Maximum quad-tree depth (default: 8)\n";
      std::cout << "  --min-size <size>       Minimum tile size (default: 4)\n";
      std::cout << "  --color-tolerance <tol> Color comparison tolerance "
                   "(default: 0)\n";
      std::cout << "  --compare               Generate both fixed-size and "
                   "quad-tree results\n";
      std::cout
          << "  --tile <WxH>            Fixed tile size (default: 32x32)\n";
  ...
  try {
    ...
    if (compareMode) {
      // 对比模式：生成两种分割结果
      std::cout << "Compare mode: generating both fixed-size and quad-tree "
                   "results...\n";
