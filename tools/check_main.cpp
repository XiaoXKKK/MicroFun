...
int main(int argc, char** argv){
    ...
    for(int i=1;i<argc;++i){
        ...
        else if(a=="-q" || a== "--quadtree"){ useQuadTree = true; }
        else if(a=="-h"){
            std::cout << "Usage: check_tool -i <resource_dir> -p posx,posy -s w,h [-q|--quadtree] [-o <output.png>]\n";
            return 0;
        }
    }
    ...
    if(!index->load(meta)){ std::cerr<<"Failed load meta: "<<meta<<"\n"; return 1; }
    ...
    if(outputPNG){
        ...
        if(!assembler.assemble(*index, vp, resourceDir, png)){ std::cerr<<"Assemble failed\n"; return 2; }
