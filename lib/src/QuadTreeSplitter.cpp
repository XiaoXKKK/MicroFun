...
void QuadTreeSplitter::subdivideNode(QuadTreeNode* node,
  ...
  if (isUniform || currentDepth >= config.maxDepth ||
      actualWidth <= config.minTileSize || actualHeight <= config.minTileSize) {
    // 标记为叶子节点
    if (isUniform) {
  ...
  if (actualWidth > 1 && actualHeight > 1) {
    ...
    for (const auto& child : node->getChildren()) {
      subdivideNode(child.get(), imageData, imageWidth, imageHeight, config,
                    currentDepth + 1);
    }
  }
}
...
void QuadTreeSplitter::collectLeafTiles(const QuadTreeNode* node,
  ...
  if (node->isLeaf()) {
    ...
    if (node->hasUniformColor()) {
      ...
      success = generateTile(imageData, imageWidth, imageHeight, x, y, width,
                             height, filePath);
    }
    ...
    // 内部节点，递归处理子节点
    for (const auto& child : node->getChildren()) {
      collectLeafTiles(child.get(), imageData, imageWidth, imageHeight, outDir,
                       tiles);
    }
  }
}
