...
std::vector<TileMeta> QuadTreeSplitter::splitQuadTree(
    ...
    unsigned char* imageData = stbi_load(inputPath.c_str(), &width, &height, &channels, 4);
    if (!imageData) {
    ...
    std::cout << "QuadTree split completed: " << tiles.size() << " tiles generated" << std::endl;
    return tiles;
}

std::vector<TileMeta> QuadTreeSplitter::split(
    const std::string& inputPath, 
    const std::string& outDir,
    int tileW, 
    int tileH) {
...
std::unique_ptr<QuadTreeNode> QuadTreeSplitter::buildQuadTree(
    ...
    std::cout << "Building quad tree with size: " << imageWidth << "x" << imageHeight << std::endl;
...
void QuadTreeSplitter::subdivideNode(
    QuadTreeNode* node, 
    const unsigned char* imageData, 
    int imageWidth, 
    int imageHeight,
    ...
    bool isUniform = colorChecker_.isUniformColor(imageData, imageWidth, 
                                                 node->getX(), node->getY(), 
                                                 actualWidth, actualHeight, uniformColor);
    
    // 终止条件：1. 颜色一致 2. 达到最大深度 3. 达到最小尺寸
    if (isUniform || currentDepth >= config.maxDepth || actualWidth <= config.minTileSize || actualHeight <= config.minTileSize) {
        // 标记为叶子节点
        if (isUniform) {
...
void QuadTreeSplitter::collectLeafTiles(
    ...
    if (node->isLeaf()) {
        ...
        if (node->hasUniformColor()) {
            ...
            success = generateTile(imageData, imageWidth, imageHeight, 
        ...
        // 内部节点，递归处理子节点
        for (const auto& child : node->getChildren()) {
            collectLeafTiles(child.get(), imageData, imageWidth, imageHeight, outDir, tiles);
        }
    }
}

bool QuadTreeSplitter::generateTile(
    const unsigned char* imageData,
    int imageWidth,
    int imageHeight,
    ...
    for (int dy = 0; dy < height; ++dy) {
        for (int dx = 0; dx < width; ++dx) {
            ...
            if (srcX < imageWidth && srcY < imageHeight) {
                // 复制像素数据
                const unsigned char* srcPixel = &imageData[(srcY * imageWidth + srcX) * 4];
    ...
    // 保存PNG文件
    int result = stbi_write_png(outputPath.c_str(), width, height, 4, tileData.data(), width * 4);
    return result != 0;
}
...
bool QuadTreeSplitter::ensureDirectoryExists(const std::string& outDir) {
    try {
        ...
        std::cerr << "Error creating directory " << outDir << ": " << e.what() << std::endl;
        return false;
    }
}

std::string QuadTreeSplitter::generateTileFileName(int x, int y, int width, int height) const {
    return "qtile_" + std::to_string(x) + "_" + std::to_string(y) + "_" + std::to_string(width) + "x" + std::to_string(height) + ".png";
}
