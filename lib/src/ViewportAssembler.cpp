...
bool ViewportAssembler::assemble(const TileIndex& index, const Viewport& vp, const string& outFile) const {
    ...
    auto blit = [&](const unsigned char* src,int sw,int sh,int stride,int dstX,int dstY){
        for(int y=0;y<sh;++y){
            ...
            for(int x=0;x<sw;++x){
                ...
                dp[3] = static_cast<unsigned char>( min(255.0f, sp[3] + dp[3]*(1-a)) );
            }
        }
    };
    // load each tile (assume current working dir contains tile files or provide relative path externally)
    for(auto &t: tiles){
        int w,h,c; unsigned char* data = stbi_load(t.file.c_str(), &w,&h,&c,4);
        if(!data){ cerr << "Failed load tile "<<t.file<<"\n"; continue; }
        int localX = t.x - vp.x;
        int localY = t.y - vp.y;
