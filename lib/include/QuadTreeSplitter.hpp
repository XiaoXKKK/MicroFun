...
class QuadTreeSplitter : public TileSplitter {
 public:
   ...
   * @return 生成的瓦片元数据列表
   */
  std::vector<TileMeta> splitQuadTree(const std::string& inputPath,
   ...
   * @return 生成的瓦片元数据列表
   */
  std::vector<TileMeta> split(const std::string& inputPath,
                              const std::string& outDir, int tileW, int tileH);
 ...
 private:
   ...
   * @return 四叉树根节点
   */
  std::unique_ptr<QuadTreeNode> buildQuadTree(const unsigned char* imageData,
   ...
   */
  void subdivideNode(QuadTreeNode* node, const unsigned char* imageData,
                     int imageWidth, int imageHeight, const Config& config,
                     int currentDepth);
   ...
   */
  void collectLeafTiles(const QuadTreeNode* node,
                        const unsigned char* imageData, int imageWidth,
                        int imageHeight, const std::string& outDir,
   ...
   * @return 是否成功生成
   */
  bool generateTile(const unsigned char* imageData, int imageWidth,
                    int imageHeight, int x, int y, int width, int height,
